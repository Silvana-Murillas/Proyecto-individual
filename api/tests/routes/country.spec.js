...
const { getRecipes } = require("../../../practica.jsx");
...
describe("GET /recipes", (done) => {
  it("should get 201", (done) => {
    request(app).get("/recipes").expect(201).then(done());
  });
...
describe("POST /recipes", (done) => {
  it("should get 201", (done) => {
    request(app)
      .post("/recipes")
      .send({ name: "Frijol", summary: "Frijol" })
      .expect(201)
      .then(done());
  });
  it("should respond with a message when recipes is create", (done) => {
    request(app)
      .post("/recipes")
      .send({ name: "Arroz", summary: "Arroz" })
      .end((error, res) => {
        if (error) {
          console.log(error);
          done(error);
        } else {
          expect(res.text).equal("Receta creada existosamente");
          done();
        }
      });
  });
...
describe("DELETE /recipes", () => {
  it("should delete a recipe", async () => {
    const recipe = await Recipe.findAll({
      where: { name: "Milanea a la napolitana" },
    });

    
    const res = await request(app).delete(
      `/recipes/${recipe[0].dataValues.id}`
    );

    expect(res.text).equal("recipe successfully deleted");
  });
  it("should throw error status(404) if the recipe can not be delete",async()=>{
    await request(app).delete(`/recipes/${1234}`).expect(404)
